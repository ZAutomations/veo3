# What If California Left the Union? - style bible

Preset: **3D Map** (`3d-map`, niche)
Format: Flow, 8s per clip, 6 clips, 48 seconds
...
## The place
**Digital Globe** - the one place this film happens in. Every clip is shot here and it
never changes: same furniture, same light, same time of day. The plate
is in `character_sheets.txt` - generate it, save it as
`character_refs/Digital Globe.jpg`, and mention it with `@Digital Globe` so the
model is handed the pixels rather than one more description of them.

A high-tech digital control room featuring a spherical 3D satellite map visualizer floating above a dark slate floor. Bright overhead key light illuminates photoreal terrain layers, ocean depth contours, and subtle vector grid lines across the surface. The room lighting, dark surrounding walls, and holographic spherical display remain static with no physical furniture, redecorating, or shift in perspective throughout.
...
1. Generate the place plate from `character_sheets.txt` and save it
   into `character_refs/` as `Digital Globe.jpg`.
2. Stage 1: `npm run agent:prompt -- stories/what_if_california_left_the_union/what_if_california_left_the_union_story.json`
